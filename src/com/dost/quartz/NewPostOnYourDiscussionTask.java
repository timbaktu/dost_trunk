...
public class NewPostOnYourDiscussionTask {
	...
	public void newDiscussionTask() {
		...
		if(genericEmail != null) {
			...
			List<DbUser> users = topicService.getUsersBelongingToDiscussionTopic(genericEmail.getConversationId()); // genericEmails conversationId is topicId, see addEntryInEmailTableForNewPost in GenericPostDAO.java
	...
	private void sendEmail(DbDostEmail email) {
		String subject = "New Reply On Discussion";
	...
	private String createEmailContent(DbDostEmail email) {
        ...
        content.append("We wanted to let you know that there is a new comment in the discussion thread <a href='http://yourdost.com/posts/list/" + email.getConversationId() + ".page'>" + topicName + "</a> on Your D.O.S.T.");
		content.append("<br><br>");
	...
	private List<DbDostEmail> getEntriesToProcess() {
		return emailService.getEmailEntriesByTypeAndStatus("NEW_POST_ON_TOPIC", "NOT_SENT");
	}
	
	private DbDostEmail getGenericEntryToExplode() {
		return emailService.getGenericEmailEntriesByTypeAndStatus("NEW_POST_ON_TOPIC", "NOT_SENT");
	}
}
